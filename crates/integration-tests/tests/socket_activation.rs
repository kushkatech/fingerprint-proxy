...
const SYSTEMD_LISTEN_PID_ENV: &str = "LISTEN_PID";
const SYSTEMD_LISTEN_FDS_ENV: &str = "LISTEN_FDS";
...
impl TestPki {
    fn generate() -> Self {
        ...
        dn.push(DnType::CommonName, "fp-socket-activation-it-ca");
        ...
        let mut leaf_params = CertificateParams::new(vec!["socket.test".to_string()]);
        ...
        dir.push(format!("fp-socket-activation-it-{id}"));
...
fn write_temp_config(contents: &str, suffix: &str) -> PathBuf {
    ...
    p.push(format!("fp-socket-activation-it-{id}{suffix}"));
    std::fs::write(&p, contents).expect("write config");
    p
}

fn start_http_stub(body: &'static str, expected_requests: usize) -> u16 {
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind");
...
fn spawn_runtime_inherited_systemd(
    config_path: &Path,
    domain_config_path: &Path,
    inherited_listener: &TcpListener,
) -> Child {
    let bin = fingerprint_proxy_bin();
    let script = format!(
        "export {listen_pid}=$$ {listen_fds}=1; exec \"{}\"",
        bin.display(),
        listen_pid = SYSTEMD_LISTEN_PID_ENV,
        listen_fds = SYSTEMD_LISTEN_FDS_ENV,
    );

    let fd = inherited_listener.as_raw_fd();
    ...
    unsafe {
        cmd.pre_exec(move || {
            // Ensure the inherited listener is available at fd 3 for systemd-style startup.
            if fd == 3 {
                let dup = dup2(fd, 4);
                if dup == -1 {
    ...
    cmd.spawn()
        .expect("spawn fingerprint-proxy (inherited systemd)")
}
...
#[tokio::test]
async fn direct_bind_mode_starts_and_serves_https_traffic() {
    let pki = TestPki::generate();
    let listener_port = reserve_local_port();
    let upstream_port = start_http_stub("direct-bind-ok", 1);
...
[[virtual_hosts]]
id = 1
match_criteria = {{ sni = [{{ kind = "exact", value = "socket.test" }}], destination = [] }}
tls = {{ certificate = {{ id = "default" }}, cipher_suites = [] }}
upstream = {{ protocol = "http", host = "127.0.0.1", port = {upstream_port} }}
...
"#
    ...
    let resp = send_https_request(addr, &pki.ca_cert_pem, "socket.test", "socket.test").await;
    assert!(resp.starts_with(b"HTTP/1.1 200\r\n"), "response: {resp:?}");
    assert_eq!(response_body(&resp), b"direct-bind-ok");
...
#[tokio::test]
async fn inherited_systemd_mode_starts_and_serves_https_traffic_from_inherited_fd() {
    let pki = TestPki::generate();
    ...
    let upstream_port = start_http_stub("inherited-ok", 1);
...
"#
    ...
    let mut child =
        spawn_runtime_inherited_systemd(&bootstrap_path, &domain_path, &inherited_listener);
    wait_for_ready(inherited_addr, Duration::from_secs(5));
    ...
    .await;
    assert!(resp.starts_with(b"HTTP/1.1 200\r\n"), "response: {resp:?}");
    assert_eq!(response_body(&resp), b"inherited-ok");
...
#[test]
fn inherited_systemd_mode_missing_env_fails_deterministically_at_startup() {
    let pki = TestPki::generate();
    let bootstrap = format!(
        r#"
listener_acquisition_mode = "inherited_systemd"
...
"#,
    ...
    let bootstrap_path = write_temp_config(&bootstrap, ".toml");
    let domain_path = write_temp_config("version = \"v1\"\n", ".toml");

    let mut child = spawn_runtime(&bootstrap_path, &domain_path);
    let status = wait_for_exit(&mut child, Duration::from_secs(5));
    assert!(!status.success(), "process unexpectedly succeeded");
    ...
    assert!(
        stderr.contains("missing required env var LISTEN_PID for systemd socket activation"),
        "stderr: {stderr}"
    );
}
